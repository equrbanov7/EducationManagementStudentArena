document.addEventListener("DOMContentLoaded", () => {
    ...
    document.body.addEventListener("click", async (e) => {
      ...
      try {
        ...
        if (!res.ok || !data.success) throw new Error(data.error || gettext("Silinmə zamanı xəta baş verdi"));
        ...
        alert(gettext("Xəta: ") + err.message);
        ...
        if (form) {
          ...
          form.addEventListener("submit", async (ev) => {
            ...
            if (!resp.ok || !result.success) {
              alert(gettext("Xəta: ") + (result.error || gettext("Yadda saxlamaq olmadı")));
              return;
            }
    ...
    if (addForm) {
      addForm.addEventListener("submit", async (e) => {
        ...
        try {
          ...
          const data = await res.json();
          if (!res.ok || !data.success) {
            alert(gettext("Xəta: ") + JSON.stringify(data.errors || data.error));
            return;
          }

          window.location.reload();
        } catch (err) {
          alert(gettext("Sistem xətası"));
        }
      });
    }
  });
