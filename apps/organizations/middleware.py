...
class OrganizationMiddleware:
    ...
    def _resolve_org_context(self, request):
        """Resolve the active organisation, its memberships and the derived
        permission set onto ``request``.

        Isolated into its own method so ``__call__`` can run it inside a
        ``transaction.atomic()`` block when ``RLS_TRANSACTION_SCOPED`` is on.
        The ``bypass_rls()`` membership reads then use ``SET LOCAL`` (transaction
        scoped) instead of a session-level ``SET`` that could otherwise linger on
        a PgBouncer-pooled backend and leak the bypass flag / tenant context into
        the next request that reuses that backend.
        """
    ...
    def __call__(self, request):
        ...
        try:
            ...
            # Faza 2 / Mərhələ 3B: transaction-pooling təhlükəsizliyi. Flaq açıq
            # olduqda org həlli (bypass_rls membership oxumaları) atomik blokda
            # icra olunur → daxildəki RLS SET-ləri SET LOCAL olur (session SET
            # deyil), beləliklə PgBouncer transaction-mode-da hovuza qaytarılan
            # backend-də bypass/tenant konteksti qalıb növbəti request-ə sızmır.
            txn_scoped = getattr(settings, "RLS_TRANSACTION_SCOPED", False)
            if txn_scoped:
                with transaction.atomic():
                    self._resolve_org_context(request)
            else:
                self._resolve_org_context(request)
            ...
            # Both ``is_superuser`` (Django's built-in flag) and ``is_superadmin``
            # (a project-level synonym) represent full cross-tenant admin access;
            # either grants the RLS bypass. Behaviour:
            #  • superuser            → user + bypass ON
            #  • org resolved         → user + bypass OFF + tenant=org
            #  • no org (secure stop) → user + bypass OFF + tenant="" (deny all)
            is_superuser = getattr(request.user, "is_superuser", False) or getattr(request.user, "is_superadmin", False)

            # Flaq açıq olduqda RLS-i request transaction-ı daxilində SET LOCAL
            # ilə tətbiq edən execute_wrapper qoşulur (PgBouncer transaction-mode
            # təhlükəsiz). Default OFF → mövcud session-scope davranış.
