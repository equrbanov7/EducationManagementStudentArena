// CodeMirror 5 show-hint addon (EMSArena vendor copy).
// Lightweight reimplementation that mirrors the official addon's public API:
//   editor.showHint(options)           // open hints
//   CodeMirror.registerHelper("hint", "name", fn)
//   CodeMirror.commands.autocomplete = function (cm) { cm.showHint(); }
// Hints are objects with `list`, `from`, `to` (positions).
// Each list entry is either a string or { text, displayText, className, hint, render }.
(function (mod) {
    ...
    function Widget(completion, data) {
        ...
        for (var i = 0; i < completions.length; ++i) {
            ...
            if (cur.render) {
                ...
                label.textContent = cur.displayText || getText(cur);
                elt.appendChild(label);
                if (cur.kind) {
                    var kindEl = ownerDocument.createElement("span");
                    kindEl.className = "CodeMirror-hint-kind";
                    kindEl.textContent = cur.kind;
                    elt.appendChild(kindEl);
                }
            }
            elt.hintId = i;
        }
        ...
        var scrolls =
            completion.options.paddingForScrollbar ? hints.scrollHeight > hints.clientHeight + 1 : false;

        // Compute overflow corrections.
        var startScroll;
        setTimeout(function () {
