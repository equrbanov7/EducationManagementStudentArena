...
| P0-5 | Dərs yükü axını mövcud deyildi (`apps/workload` yox) | PHASE2 | IN-PROGRESS — worktree `agent-abbdb0f75634105ee` (sessiya limiti ilə yarımçıq) |
...
| P1-7 | Tələbə siyahısını yükləmək üçün heyət yolu yoxdur (`import_users_from_excel` komandası prod-da `MANAGEMENT_COMMAND_ENVIRONMENT` ilə söndürülüb; RİM mərkəzində «yarat/idxal» yoxdur) | PHASE1 provisioning | OPEN → 2-ci dalğa |
