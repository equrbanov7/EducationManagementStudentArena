...
| **U3+** (bitdi) | **Yekun qiymət + təkrar imtahan**: `FinalGrade` (yekun imtahan balı) + `ResitRecord` (səbəb: qayıb/ümumi/imtahan) + `AssessmentScheme.pass_threshold`/`min_final_exam_score` (migration 0012/0013 + RLS). Servis: giriş balı + imtahan → hərf/keçdi-kəsildi; kəsilən → resit; resit balı imtahanı əvəz edir + qayıb blokunu qaldırır; `publish_offering` finalizasiya + audit. UI: müəllim "Yekun nəticə" grid + "Yekunlaşdır"; tələbə nəticə + resit hüququ | orta | ✔ |
...
| **U5** (bitdi) | **Transkript + GPA**: `transcript.build_student_transcript` — enrollment-lər semestr (period) üzrə qruplaşır, hər fənn `finals.compute_final_result`-dən hərf/GPA nöqtəsi alır; **kredit-çəkili** kumulyativ GPA (Σ gpa×kredit / Σ kredit; keçən+kəsilən daxil, qiymətlənməyən sonra); tələbə kabinetində "Transkript" bölməsi (semestr cədvəlləri + GPA kartı + toplanmış kredit). Yeni model yox — mövcud `FinalGrade` üzərində oxu. | yüksək | ✔ |
| **U5+** | Status state-machine (aktiv / akademik məzuniyyət / xaric / məzun) + keçid audit (P3-2 §6-8) | orta | sonra |
