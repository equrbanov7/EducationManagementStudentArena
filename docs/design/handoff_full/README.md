...
2. Jurnal yaradılması təsdiqlənmiş sillabus olmadan **bloklanır** (403 + səbəb kodu); icazə varsa yalnız read-only görünüş qaytarılır. **Tətbiq (2026-09-14):** bu qayda qeyd-şərtsiz deyil, **təşkilat siyasətidir** — `apps/registrar/journal_policy.py`, açar `Organization.settings["journal"]["require_approved_syllabus"]`, **default: söndürülü** (köçürülmüş bazada açılışların əksəriyyətinin sillabusu yoxdur; söndürülü halda yalnız `syllabus_notice` xəbərdarlıq banneri qalır). Açıq olduqda dərs yaratma `SyllabusGateError` atır, jurnal POST-u 403 + səbəb kodu qaytarır, görünüş read-only olur (`can_edit=False`).
