...
| **Akademik təqvim** | ✅ **İCRA OLUNDU (U11)** | Plana uyğun — yeni ağır model YOX: `AcademicPeriod`-a 4 opsional tarix sahəsi (`registration_start/end`, `exam_session_start/end`, miqrasiya organizations.0014) + `registration_state`/`exam_session_state` helper-ləri (open/upcoming/closed/None). `/jurnal/teqvim/` timeline görünüşü (bütün org üzvləri): semestr kartları + pəncərə statuslu badge-lər. Dərs cədvəli səhifəsindən + ⌘K-dan link. Admin fieldset + seed demo tarixləri. Testlər: `test_calendar.py` (4). Qalan: qeydiyyat pəncərəsinin seçmə-fənn axınına məcburi tətbiqi (gələcək iterasiya). |
| **Analitika/Hesabat paneli** | ✅ **İCRA OLUNDU (U10)** | `/jurnal/analitika/` — `apps/registrar/analytics.py` (batched aqreqasiya, qeydiyyat sayından asılı olmayaraq SABİT ~7 sorğu; `compute_final_result` ilə uyğunluq testi ilə kilidlənib) + `analytics_views.py` (RBAC: dekan/kafedra/admin). Semestr seçimi, stat kartlar (tələbə, qeydiyyat, keçid %, kredit-çəkili orta GPA, qayıb %), proqram/qrup cədvəlləri, "diqqət tələb edən fənlər" (kəsilmə faizi bar-ları). Sidebar + ⌘K linkləri. Testlər: `test_analytics.py` (10). Qalan: unit-scoped görünüş (dekan yalnız öz fakültəsi) — tenant konfiqurasiyası gələndə. |
