...
STRINGS = {
    ...
    # ── Qrup reyestri: alt qruplara bölmə (sahib 2026-09-20) ──────────────────
    ("accounts.groups", "Alt qruplara böl"): ("Split into subgroups", "Разделить на подгруппы", "Alt gruplara böl"),
    ("accounts.groups", "%(name)s qrupunu alt qruplara böl"): ("Split group %(name)s into subgroups", "Разделить группу %(name)s на подгруппы", "%(name)s grubunu alt gruplara böl"),
    ("accounts.groups", "Ana qrupu alt qruplara böl"): ("Split the parent group into subgroups", "Разделить основную группу на подгруппы", "Ana grubu alt gruplara böl"),
    ("accounts.groups", "Tələbələr əvvəlcə bərabər bölünür; siyahılar arasında keçirə, alt qrupları adlandıra bilərsiniz. Seçilməyənlər ana qrupda qalır. Hər köçürmə rəsmi xidmətlə yazılır — tarixçə silinmir."): (
        "Students are first split evenly; you can move them between lists and name the subgroups. Unselected students stay in the parent group. Every move is written by the official transfer service — history is kept.",
        "Сначала студенты делятся поровну; их можно перемещать между списками и называть подгруппы. Невыбранные остаются в основной группе. Каждый перевод выполняется официальной службой — история сохраняется.",
        "Öğrenciler önce eşit bölünür; listeler arasında taşıyabilir ve alt grupları adlandırabilirsiniz. Seçilmeyenler ana grupta kalır. Her taşıma resmi hizmetle yazılır — geçmiş silinmez.",
    ),
    ("accounts.groups", "Alt qrupları yarat"): ("Create subgroups", "Создать подгруппы", "Alt grupları oluştur"),
    ("accounts.groups", "Tələbələr yüklənir…"): ("Loading students…", "Загрузка студентов…", "Öğrenciler yükleniyor…"),
    ("accounts.groups", "Bu qrupda aktiv tələbə yoxdur."): ("This group has no active students.", "В этой группе нет активных студентов.", "Bu grupta aktif öğrenci yok."),
    ("accounts.groups", "Ana qrupda qalır"): ("Stays in the parent group", "Остаётся в основной группе", "Ana grupta kalır"),
    ("accounts.groups", "tələbə"): ("students", "студ.", "öğrenci"),
    ("accounts.groups", "Seçilmişləri bura keçir"): ("Move selected here", "Переместить выбранных сюда", "Seçilenleri buraya taşı"),
    ("accounts.groups", "Hamısını seç"): ("Select all", "Выбрать всех", "Tümünü seç"),
    ("accounts.groups", "Seçimi ləğv et"): ("Clear selection", "Снять выбор", "Seçimi temizle"),
    ("accounts.groups", "Alt qrupun adı"): ("Subgroup name", "Название подгруппы", "Alt grup adı"),
    ("accounts.groups", "Tələbə siyahısı yüklənmədi — yenidən cəhd edin."): ("Student list failed to load — try again.", "Список студентов не загрузился — попробуйте ещё раз.", "Öğrenci listesi yüklenemedi — tekrar deneyin."),
    ("accounts.groups", "Ən azı iki alt qrupda tələbə olmalıdır."): ("At least two subgroups must contain students.", "Минимум две подгруппы должны содержать студентов.", "En az iki alt grupta öğrenci olmalıdır."),
    ("accounts.groups", "Hər alt qrupun adı olmalıdır."): ("Every subgroup needs a name.", "У каждой подгруппы должно быть название.", "Her alt grubun adı olmalıdır."),
    ("accounts.groups", "Alt qrup sayı"): ("Number of subgroups", "Количество подгрупп", "Alt grup sayısı"),
    ("accounts.groups", "Bərabər böl"): ("Split evenly", "Разделить поровну", "Eşit böl"),
    ("accounts.groups", "Sütunlarda tələbəni işarələyib başqa sütunun «Seçilmişləri bura keçir» düyməsi ilə keçirin. Ana qrupda qalan tələbələr köçürülmür."): (
        "Tick students in a column and move them with another column's “Move selected here” button. Students left in the parent group are not transferred.",
        "Отметьте студентов в столбце и переместите их кнопкой «Переместить выбранных сюда» другого столбца. Оставшиеся в основной группе не переводятся.",
        "Bir sütunda öğrencileri işaretleyip başka sütunun «Seçilenleri buraya taşı» düğmesiyle taşıyın. Ana grupta kalanlar taşınmaz.",
    ),
    ("accounts.groups", "Alt qrup bölgüsü"): ("Subgroup split", "Разбиение на подгруппы", "Alt grup dağılımı"),
    ("accounts.groups", "Səbəb"): ("Reason", "Причина", "Neden"),
    ("accounts.groups", "Məs.: laboratoriya dərsləri üçün alt qruplara bölünür"): ("e.g. split into subgroups for laboratory classes", "напр.: разделение на подгруппы для лабораторных", "örn.: laboratuvar dersleri için alt gruplara bölünür"),
    ("accounts.groups", "Səbəb hər tələbənin köçürmə qeydinə və audit jurnalına yazılır."): ("The reason is written to each student's transfer record and the audit log.", "Причина записывается в запись перевода каждого студента и в журнал аудита.", "Neden, her öğrencinin taşıma kaydına ve denetim günlüğüne yazılır."),
    ("accounts.groups", "Alt qrup bölgüsü oxunmadı — səhifəni yeniləyib yenidən cəhd edin."): ("The subgroup split could not be read — refresh the page and try again.", "Не удалось прочитать разбиение — обновите страницу и попробуйте снова.", "Alt grup dağılımı okunamadı — sayfayı yenileyip tekrar deneyin."),
    ("accounts.groups", "Alt qrup adları bir-birindən fərqli olmalıdır."): ("Subgroup names must differ from each other.", "Названия подгрупп должны отличаться.", "Alt grup adları birbirinden farklı olmalıdır."),
    ("accounts.groups", "Bir tələbə yalnız bir alt qrupda ola bilər."): ("A student can be in only one subgroup.", "Студент может быть только в одной подгруппе.", "Bir öğrenci yalnızca bir alt grupta olabilir."),
    ("accounts.groups", "Ən azı 2 alt qrup olmalıdır."): ("At least 2 subgroups are required.", "Нужно минимум 2 подгруппы.", "En az 2 alt grup olmalıdır."),
    ("accounts.groups", "Ən çoxu 6 alt qrup yaratmaq olar."): ("At most 6 subgroups can be created.", "Можно создать не более 6 подгрупп.", "En fazla 6 alt grup oluşturulabilir."),
    ("accounts.groups", "Arxivlənmiş qrup bölünmür."): ("An archived group cannot be split.", "Архивную группу нельзя разделить.", "Arşivlenmiş grup bölünemez."),
    ("accounts.groups", "«%(name)s» adlı qrup artıq var — başqa ad seçin."): ("A group named “%(name)s” already exists — choose another name.", "Группа «%(name)s» уже существует — выберите другое название.", "«%(name)s» adlı grup zaten var — başka bir ad seçin."),
    ("accounts.groups", "Heç bir tələbə alt qrupa yerləşdirilməyib."): ("No student has been placed in a subgroup.", "Ни один студент не помещён в подгруппу.", "Hiçbir öğrenci alt gruba yerleştirilmedi."),
    ("accounts.groups", "Seçilmiş tələbələrdən bəziləri bu qrupun aktiv tələbəsi deyil — siyahını yeniləyin."): ("Some selected students are not active students of this group — refresh the list.", "Некоторые выбранные студенты не являются активными студентами этой группы — обновите список.", "Seçilen bazı öğrenciler bu grubun aktif öğrencisi değil — listeyi yenileyin."),
}  # fmt: skip
